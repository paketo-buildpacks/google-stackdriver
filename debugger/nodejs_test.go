...
func testNodeJS(t *testing.T, context spec.G, it spec.S) {
	...
	context("Build", func() {
		...
		it.Before(func() {
			var err error
			ctx.Layers.Path, err = ioutil.TempDir("", "debugger-nodejs-build-layers")
		...
		it("contributes NodeJS agent", func() {
			dep := libpak.BuildpackDependency{
				URI:    "https://localhost/stub-cloud-debugger-agent.tgz",
				SHA256: "c3ecfa1e2daa29db419b063dec9ea20108923e406d9ab7a35318f6f14f615dc6",
			}
			dc := libpak.DependencyCache{CachePath: "testdata"}

			n := debugger.NewNodeJSBuild(dep, dc, &libcnb.BuildpackPlan{})
			n.Executor = executor
			...
			Expect(execution.Args).To(Equal([]string{"install", "--no-save",
				filepath.Join("testdata",
					...
					"stub-cloud-debugger-agent.tgz"),
			}))

			Expect(layer.LaunchEnvironment[fmt.Sprintf("%s.default", debugger.NodePath)]).
	...
	context("Launch", func() {
		...
		it.Before(func() {
			var err error
			l.ApplicationPath, err = ioutil.TempDir("", "debugger-nodejs-launch-application")
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(os.Setenv(debugger.NodePath, "test-path")).To(Succeed())
		...
		it.After(func() {
			...
			Expect(os.Unsetenv(debugger.NodePath)).To(Succeed())
		...
		it("returns error if BPI_GOOGLE_CLOUD_DEBUGGER_NODE_PATH is not set", func() {
			Expect(os.Unsetenv(debugger.NodePath)).To(Succeed())

			_, err := l.Execute()
			Expect(err).To(MatchError("$BPI_GOOGLE_CLOUD_DEBUGGER_NODE_PATH must be set"))
		})
		...
		context("binding", func() {
			...
			context("@google-cloud/debug-agent already required", func() {
...
test`), 0644)).
				...
				it("does not contribute require('@google-cloud/debug-agent')", func() {
					_, err := l.Execute()
					Expect(err).NotTo(HaveOccurred())
...
test`))
			...
			context("@google-cloud/debug-agent not already required", func() {

				it("contributes require('@google-cloud/debug-agent')", func() {
					_, err := l.Execute()
					Expect(err).NotTo(HaveOccurred())
					...
					Expect(string(b)).To(Equal(`require('@google-cloud/debug-agent').start({
