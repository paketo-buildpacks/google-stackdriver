...
type NodeJSDebuggerAgent struct {
...
func (n NodeJSDebuggerAgent) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	...
	if !regexp.MustCompile(`require\(['"]@google-cloud/debug-agent['"]\)`).Match(c) {
		n.Logger.Header("Requiring '@google-cloud/debug-agent' module")

		if err := ioutil.WriteFile(file, append([]byte("require('@google-cloud/debug-agent').start();\n"), c...), 0644); err != nil {
